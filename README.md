...
kube-applier will try to fetch a Service Account under each namespace (as defined by the environment variable `SERVICE_ACCOUNT_NAME`) and will use it to run kubectl commands. It will error for all namespaces that do not contain a matching SA.
...
* `SERVICE_ACCOUNT_NAME` - (string) The name of the service account used when applying. Defaults to `kube-applier`.
