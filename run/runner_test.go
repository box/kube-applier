...
type runnerTestCase struct {
	batchApplier run.BatchApplierInterface
...
func TestRunnerRun(t *testing.T) {
	...
	batchApplier := run.NewMockBatchApplierInterface(mockCtrl)
...
func runAndAssert(t *testing.T, tc runnerTestCase) {
	assert := assert.New(t)
	r := run.Runner{tc.batchApplier, tc.factory, tc.repo, tc.clock, tc.metrics, ""}
	result, err := r.Run()
